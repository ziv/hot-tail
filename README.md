...
All art and audio are generated at boot — aircraft are lofted from cross-sections with airfoil-section wings, glass canopies, metal nozzles and procedural panel lines (`src/render/airframe.ts`, `models-aircraft.ts`; preview them at `/tools/models.html` in `pnpm dev`), the rest are procedural placeholders — no asset downloads (the whole production build is ≈ 195 KB brotli). Two visual styles are selectable in Settings → Graphics: modern 3D, and a retro sprite-scaling mode that pre-renders every model into angle-indexed sprite atlases at boot and draws entities as billboards at ~288 lines with scanlines.
